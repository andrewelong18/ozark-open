...
| `docs/LANDING_PAGE_OVERHAUL.md` | Anything touching the logged-out landing page (`app/page.tsx`). The running brief for the overhaul — current state, stack constraints, each reference/preference Andrew has supplied, and the open questions still blocking a plan. Context collection is in progress; don't build from it until the brief is closed. |
