...
| `docs/LANDING_PAGE_OVERHAUL.md` | Anything touching the logged-out landing page (`app/page.tsx`). The running brief for the overhaul — current state, stack constraints, each reference/preference Andrew has supplied, and the open questions still blocking a plan. Item 2 is the decision log (24 recommendations, Andrew's verdict on each) and is operative. Direction is approved; blocked on assets, so don't build yet. |
