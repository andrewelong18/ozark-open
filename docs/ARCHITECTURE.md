...
- **Bettor approval (Sprint 16).** An onboarded member can browse the bet menu but can't place bets until an admin approves them on `/admin/people` (Sprint 20; `/admin/participants` redirects there). Approving verifies/corrects the `display_name` (for import name-matching), sets the Phase 1 / Phase 2 entries + player flag (prefilled from the member's one-time entry request, Sprint 30), and **creates the `tournament_participants` row** — a live row is the eligibility gate (PRD §12 A11/A12, refined by A13: the row must also have `revoked_at IS NULL`). This replaces the manual Studio row-add; there is no `betting_enabled` column. **Revoking is a soft revoke** (Sprint 21): it stamps `revoked_at` and keeps the row, because the row carries the phase entries and they are pool inputs — deleting it moved money. **Eligibility is per phase since Sprint 30** (ADR 0002): a live row **and** an entry for the bet's phase.
