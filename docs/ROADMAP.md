...
| [24](sprints/sprint-24.md) | Betting menu UX (handicap badges · collapsible reveal · filter refactor · pick ordering) | 🔲 cut Jul 31, 2026. Presentation only, no schema. Includes a real correctness fix: `/bets` fetches picks with no `ORDER BY` at any layer, so order is undefined and an upsert can reshuffle it (#102–#105) | none |
