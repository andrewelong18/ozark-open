...
| [9](sprints/sprint-9.md) | Polish & group dry run | 🔶 dry run **run Jul 31, 2026** with Pat against production — the full lifecycle (menu open → wagers → close → results → Phase 2 → payouts) completed, and the pool reconciled to the cent: entry fees $425 − voided stakes $32 = **pool $393**, all 87 picks settled, zero pending. Produced 21 findings → Sprints [21](sprints/sprint-21.md)–[25](sprints/sprint-25.md) and issues #90–#109; session record in [`docs/dry-run/ISSUE_LOG.md`](dry-run/ISSUE_LOG.md). **Still open:** Act 10.3 reconciliation against Pat's workbook, and the polish half of the sprint | 0–7 (8 nice-to-have) |
| [10](sprints/sprint-10.md) | ★ Admin roster & registration status | 🔶 code complete Jul 25, 2026 (`tournament_invites` + `admin_auth_activity()` RPC, `lib/roster.ts` status derivation + relative-time util — unit-tested + built locally; `/admin/roster` attention strip + table, admin-panel entry point); **prod SQL applied + verified Jul 25, 2026** — browser walkthrough pending. **Page superseded by Sprint 20** (Jul 26): `/admin/roster` redirects to `/admin/people`; the schema and `lib/roster.ts` carry forward unchanged | none hard (bonus) |
...
| Dry-run defect fixes | The five ways the app was wrong when Pat drove it | 🔶 Four of five shipped Aug 7, 2026 (soft revoke + pool arithmetic, stake-rejection messages, dry-run tooling, gameplan errata). The avatar-upload RLS P0 (#90) is diagnosed, not fixed — it needs one run against prod. Prod migration #113 is the gating step for the revoke fix | 21 | before Aug 28 |
| ★ Bonus wish list | Admin roster · bet-state snapshots · animation & Jake celebration · funny ads · announcement banner · self-serve profile page · first-run onboarding + bettor-approval · bet-slip review & confirmation · player profile modals | 🔶 Sprint 15 (profile page) code complete Jul 19, 2026; Sprint 16 (onboarding + bettor-approval) code complete Jul 20, 2026; Sprint 18 (player profile modals) code complete Jul 23, 2026; Sprint 10 (admin roster) code complete + prod SQL applied Jul 25, 2026, its page absorbed into the Sprint 20 people console Jul 26; rest not started (never blocks MVP) | 10–18 | as time allows, before Aug 28 |
