...
| [21](sprints/sprint-21.md) | Dry-run bug fixes (avatar RLS · revoke destroys the entry fee · silent $0 rejection · tooling) | 🔶 cut Jul 31, 2026; **four of five items shipped Aug 7, 2026** — the money P0 (#91) is fixed as a **soft revoke** (`revoked_at`; eligibility = row exists AND not revoked, PRD §12 **A13**; fee and wagers now leave and return together), the silent $0 rejection (#92), both tooling bugs (#94, #95 — the CTE collision was in four files, not two) and the gameplan errata (#109) are done. Unit-tested (195 pass), `local-db-verify.sh` + `dry-run-verify.sh` both green, same reconciliation as Jul 31 ($425 − $32 = $393). **Still open:** the avatar RLS P0 (#90) is diagnosed with a probe script but needs prod credentials to run, which blocks #93; **prod migration applied + verified Aug 7** (#113 closed — the merge deployed code that reads `revoked_at` before the column existed, so this became urgent rather than routine); browser round trip (#114) and one live magic-link run (#115) are still owed | none |
| [22](sprints/sprint-22.md) | Rule reinforcement & import guards (tournament-wide pick minimum · status/result validation · chase-query rewrite) | 🔲 cut Jul 31, 2026. **The ripple sprint** — Pat's Act 4.17 decision (min 5 across both phases, max 10 per phase, checked only before Phase 2 close) moves schema, `lib/validation.ts`, `phase-compliance.sql`, PRD §7/§8.1, ADR 0001 and the unit tests together. Also adds the guard that would have caught the results-on-an-open-book upload (#96–#98) | none hard; land 21 first |
