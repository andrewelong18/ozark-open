...
| [21](sprints/sprint-21.md) | Dry-run bug fixes (avatar RLS · revoke destroys the entry fee · silent $0 rejection · tooling) | 🔲 cut Jul 31, 2026 from the full-lifecycle dry run with Pat. Two P0s: onboarding's photo step fails with an RLS violation (#90), and revoking access deletes the entry fee while the bettor's wagers stay in the pool, so `poolTotal()` silently shrinks (#91). Ship before anything else from that session | none |
