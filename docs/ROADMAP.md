...
| [22](sprints/sprint-22.md) | Rule reinforcement & import guards (tournament-wide pick minimum · status/result validation · chase-query rewrite) | ✅ Code complete + fully verified locally Aug 8, 2026 (#96–#98 all four items: `min_picks_per_tournament` migration, `checkPickMinimum()`, both import guards, phase-aware chase query with a one-line "text these people" answer; PRD §12 A14/A15 and ADR 0001 §10 amended; `OUTSTANDING_DECISIONS` §2 closed). **Prod migration not yet applied** — no Supabase token in the agent environment ([#117](https://github.com/andrewelong18/ozark-open/issues/117)); it is additive, so merge order is not load-bearing | none hard; land 21 first |
| [23](sprints/sprint-23.md) | Admin control surface (editable display names · house-rules editor · add a member and bet on their behalf) | 🔲 cut Jul 31, 2026. **Expands the admin-UI scope** that was out of scope until that date. The bet-on-behalf item is the largest from the session and needs its own spec pass — every §7 rule must evaluate against the bettor, not the acting admin (#99–#101) | none hard; builds on 20 |
