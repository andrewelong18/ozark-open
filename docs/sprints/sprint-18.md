...
- [ ] **Deferred — leaderboard.** `/leaderboard` names are raw Google-Sheet strings with no FK to `users`, so they can't open a profile without name-to-user matching. Out of scope here; revisit if that matching is ever worth building — filed as [#75](https://github.com/andrewelong18/ozark-open/issues/75), still open. *(Deliberately unticked: this is the one item of the sprint that was cut, not finished.)*
