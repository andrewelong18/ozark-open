...
- [x] Per-placement receipt on place **and** edit: the snapshotted `odds_at_placement` + stake shown as a locked-odds confirmation under the row ("✓ Locked in · +450 · $5 · odds locked at placement"); cleared on remove. Surfaces data the API already returns. *(`bet-placement-card.tsx` — the `Receipt` type is kept deliberately separate from the row's live odds, so the receipt keeps showing what was locked even after the line is repriced. **Reworded Aug 31, 2026** to "✓ Locked in $5 at +110" and merged onto one line with the remove link: the stake box's "Bet placed" caption, the receipt's second odds chip and the "· odds locked at placement" tail were each saying something the row already said, and the remove link stacked under the stake box left a 44px column of air above the receipt.)*
