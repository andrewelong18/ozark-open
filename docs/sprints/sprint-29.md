...
4. **`sweep_bets()` was not actually being checked for the bug that killed the restore button.** Merging `main` brought in `f17f1e4`/`9acb273` — Supabase preloads `pg-safeupdate` on the `authenticator` role PostgREST connects as, so a WHERE-less `DELETE` inside plpgsql is rejected at parse time even under `SECURITY DEFINER`, which left `/admin/snapshots`' restore dead in production. That matters here because Sprint 29's whole defence of the hard delete is *"the snapshot puts the wagers back."* `sweep_bets()`'s three deletes do carry `WHERE … = ANY(…)`, so it was never affected — **but the new repo-wide check could not see them**: it requires a statement fragment to *begin* with the DML, and all three sit inside `WITH gone AS ( DELETE … )`. Proven on a bare cluster, both directions: under the old split a CTE-wrapped WHERE-less delete is not flagged, under the new one it is, and the same statement with a `WHERE` still isn't. The split now also breaks on `IF … THEN`, which had the identical hole. Widening it is a change to `main`'s check, taken rather than filed because a check that passes for the wrong reason is worse than no check — and CTE-wrapped deletes are now house style in this repo, which this sprint is what made true.
5. **The grants manifest also picked up pre-existing drift** — `apply_player_profile_seed()`, added Sept 9 in 2aff293 without regeneration. It is a `SECURITY DEFINER` trigger function on the default `PUBLIC` ACL, the same shape as `handle_new_user()` and `guard_users_self_update()` already in the manifest, so this records existing state rather than a new grant. Filed for review alongside #213.
