...
| `opened_at` | `timestamptz` | First time the bet was seen `open`, stamped by the importer **on the transition only** (`20260831000000`). The activity feed's "Phase N is open" event reads the earliest one in a phase. `created_at` cannot stand in for it: Phase 2 bets ship `hidden` in early uploads and are upserted by `sheet_bet_id`, so the row keeps a `created_at` from weeks before it opened. NULL = never observed opening. |
| `created_at` | `timestamptz` NOT NULL DEFAULT `now()` | |
...
- **`activity_placements(tournament, limit)`** (`20260831000000`): the dashboard activity feed's read, and the same boundary again from the other direction. RLS hides other members' placements until the bet closes, which is correct and is exactly what makes a live feed impossible without help — so this `SECURITY DEFINER` function lifts it for **five columns and no more**: `(id, user_id, display_name, avatar_url, created_at)`. No `pick_id`, no `amount`, no `odds_at_placement`, no `placed_by_user_id`, and no join that would reveal which bet a row belongs to; soft-deleted rows are excluded, so a removal is invisible rather than newsworthy. That column list *is* the privacy model for the feature (PRD §12 A16) — adding to it publishes every member's position mid-phase — so `scripts/activity-rls-roundtrip.ts` asserts the function's exact result signature on every `local-db-verify` run, alongside the behavioural checks. Gated internally on `auth.uid() IS NOT NULL`, `EXECUTE` revoked from `anon` and granted to `authenticated`. Names come from `public.users`, which is authenticated-read-all already, so the join exposes nothing new.
