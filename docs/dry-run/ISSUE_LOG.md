...
| 5 | Avatar upload fails with an RLS violation | ✅ Fixed Aug 8, 2026 (`3715725` — `lib/avatar.ts`), diagnosis corrected and browser-covered Aug 9 ([#90](https://github.com/andrewelong18/ozark-open/issues/90)). The "race against cookie hydration" explanation was wrong — supabase-js already awaits `getSession()` on every storage request — and the bucket policies were verified correct against production. `e2e/avatar-upload.spec.ts` now proves the upload end to end through real RLS. Closed Aug 9; the prod browser check is [#145](https://github.com/andrewelong18/ozark-open/issues/145) and the bucket's missing size/MIME limits are [#144](https://github.com/andrewelong18/ozark-open/issues/144). Row 6 (crop/remove) is unblocked |
