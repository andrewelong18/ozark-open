...
| A19 | **The bet menu's primary axis is the phase, not the bet's status** (Andrew, Sept 7, 2026 — Sprint 26, second dry run with Pat). `/bets` opened on an Open \| Closed toggle, and Sprint 24 / #104 had made that a deliberate **partition**: status was a view rather than a filter, so a closed bet and an open bet never sat in one list looking alike. Pat's read in September was that the control names the wrong thing — the weekend is organised around Phase 1 and Phase 2, and the app has carried `phase` through every layer since Sprint 1 without ever rendering it. So the toggle becomes Phase 1 \| Phase 2, and the status badge stops describing the whole book and describes the selected phase (`phaseState()`, which already returned exactly the three states needed). **This knowingly gives back what #104 bought**, so three things pay for it without adding a second control: open bets sort above closed ones inside each group, open bets get a status badge for the first time (only closed and resolved cards carried one, which in a mixed list is the exact ambiguity #104 was about), and the past-deadline-but-not-yet-closed state — which rendered with **no badge at all** for ~10 hours on Thursday and again on Saturday — gets one. What is preserved is the property that actually mattered: every round and category chip is still derived from the bets in the current view, so **no selectable option can empty the page**. The tabs are a fixed pair rather than derived from the tree, because Phase 2 ships `hidden` and Pat specifically asked for a tab that says it isn't open yet. Nothing about `bets.status`, `wageringOpen()` or the phase clock changed — ADR 0001 §5a stands. |
