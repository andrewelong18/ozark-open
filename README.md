...
- **Register / approve a participant:** use **`/admin/participants`** (Sprint 16), not Studio — approving a member sets their `entry_fee` + `is_player` and creates the `tournament_participants` row that grants betting access. (The row can still be hand-edited in Studio if ever needed; the app's gate is simply whether it exists.) Its read-only companion is **`/admin/roster`** (Sprint 10): who still needs to register, who registered but can't bet yet, who's ready, who's an admin, and when each person last logged in.
- **Enter the expected roster:** `tournament_invites` → one row per person you expect (`tournament_id` + `email`, optional `invited_name`). No account needed — that's the point: `/admin/roster` matches an invite to a member by email (case-insensitive) the moment they log in, so you can see who hasn't shown up yet. These rows are **not** participants and never touch pool math.
