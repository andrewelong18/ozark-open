...
3. **If the upload pauses to ask, read that panel carefully** (Sprint 29). Since the sheet is the menu in *both* directions, an upload also deletes the bets and picks it no longer lists — so whenever there is something to delete, it stops and shows you what, before writing anything and before taking a save state. It compares only against **the phases your file actually contains**, so a Phase 2-only sheet can't reach Phase 1. Rows nobody has bet on sit behind one button; **rows carrying wagers are kept unless you say otherwise**, on a separate button, with the bettors named — clearing those deletes their wagers for good, leaves the pool unchanged (it's the sum of entry fees, not of wagers), drops those members below their entry fee, and tells them nothing, so you'll be texting them. **"Import without deleting"** does what every upload did before this: adds and updates only.
4. Read the **import report**:
   ...
   - **Deleted / kept** — what the sweep removed, and anything the sheet dropped that was left in place because it carries wagers.
...
- **Link an unmatched pick to a player:** prefer fixing the member's display name on `/admin/people` so it matches the sheet, then re-upload. That also puts them on `/roster`. A hand-set link (`bet_picks` → `player_user_id`) works too, and the importer keeps it on every future upload **only while the pick's label still names the same person**. A `pick_id` the sheet reuses for someone else drops the old link (Sept 24, 2026). To check every link at once, paste `docs/admin/pick-links-check.sql` into the SQL editor.
